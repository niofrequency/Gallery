...
export default function App() {
  ...
  const handleRename = async (image: GalleryImage, newName: string) => {
    ...
    // Update selectedImage state if it's currently open to prevent visual lag
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-2.5 sm:gap-3">
                {filteredAndSortedImages.map((image) => (
                  <div
                    key={image.id}
                    onClick={() => setSelectedImage(image)}
                    className="relative group rounded-none border border-[#222] bg-[#0A0A0A] overflow-hidden cursor-pointer shadow-none transition duration-350 hover:border-neutral-500 aspect-square flex flex-col justify-end"
                  >
                    {/* Image underlay */}
                    {image.contentType.startsWith("video/") ? (
                      <video
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-black/25 to-transparent transition-all duration-300" />
                    ...
                    <div className="relative p-5 flex flex-col gap-2 z-10">
                      ...
                      <h4 className="text-sm font-medium text-white group-hover:text-orange-400 transition truncate">{image.name}</h4>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="flex flex-col gap-3.5">
                {filteredAndSortedImages.map((image) => (
                  <div
                    key={image.id}
                    onClick={() => setSelectedImage(image)}
                    className="group rounded-none border border-[#222] bg-[#0A0A0A] p-4 hover:bg-[#151515] hover:border-neutral-700 cursor-pointer transition flex gap-5 items-center overflow-hidden"
                  >
